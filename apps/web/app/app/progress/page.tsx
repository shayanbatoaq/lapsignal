...
export default async function ProgressPage() {
  ...
  const chronological = [...sessions].sort((a,b)=>a.started_at.localeCompare(b.started_at));
  const firstBest = chronological[0]?.metrics.pace.best_lap_ms;
  const latestBest = chronological.at(-1)?.metrics.pace.best_lap_ms;
  const improvementMs = firstBest != null && latestBest != null ? firstBest - latestBest : null;
  ...
  return <>
    ...
    <div className="dashboard-grid">
      <div className="span-3"><MetricCard icon={Archive} label={showcase?"Programme sessions":"Stored sessions"} value={`${sessions.length}`} detail={showcase?"representative history":"recorded locally"}/></div>
      <div className="span-3"><MetricCard icon={Timer} label={showcase?"Pace improvement":"Recorded laps"} value={showcase&&improvementMs!=null?`${(improvementMs/1000).toFixed(3)} s`:`${totalLaps}`} detail={showcase?"baseline to Practice 03":`${cleanLaps} classified clean`}/></div>
      <div className="span-3"><MetricCard icon={Gamepad2} label="Input contexts" value={`${inputContexts}`} detail={showcase ? "representative controller context" : "controller and wheel"}/></div>
      <div className="span-3"><MetricCard icon={ShieldCheck} label="Evidence chain" value="Matched" detail={showcase?"fictionalized fixture provenance":"stored provenance on every run"}/></div>
