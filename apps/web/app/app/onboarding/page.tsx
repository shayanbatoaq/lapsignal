...
const steps = [
  ...
  { key: "primary_interest", title: "What do you want to develop?", copy: "This sets the default context for your own recorded sessions.", options: [["f1","F1-style racing"],["gt3","GT3"],["endurance","Hypercars and endurance"],["mixed","A mixed programme"]] },
...
export default function OnboardingPage() {
  ...
  return <div className="onboarding"><div className="onboarding-progress" aria-label={`Step ${step+1} of 6`}>{Array.from({length:6},(_,index)=><span className={index<=step?"active":""} key={index}/>)}</div><section className="onboarding-card">
    ...
    {step===4 && <><p className="eyebrow">Operating modes · step 5</p><h2>Use LapSignal with recorded evidence.</h2><p>The collector works locally without paid services or cloud credentials.</p><div className="choice-grid"><div className="choice selected"><Radio size={20}/><strong>Live</strong><span>Native Windows UDP listener on port 20777.</span></div><div className="choice"><RotateCcw size={20}/><strong>Replay</strong><span>Explicitly reprocess a local capture you select.</span></div><div className="choice"><CheckCircle2 size={20}/><strong>Saved</strong><span>Review finalized sessions while the collector is offline.</span></div></div></>}
